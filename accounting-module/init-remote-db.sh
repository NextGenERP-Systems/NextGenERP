#!/usr/bin/env bash
# ==============================================================================
# NextGen ERP - Initialize Remote Accounting Database on Cloud VM
...
echo "======================================================================"
echo "  NextGen ERP - Initializing Remote Accounting DB on $INSTANCE_NAME   "
echo "======================================================================"
...
gcloud compute scp "$SCRIPT_DIR/database/schema.sql" "$SCRIPT_DIR/database/seed-data.sql" \
  "${INSTANCE_NAME}:/tmp/" --zone="$ZONE" --project="$PROJECT_ID"

# 3. Apply schema
echo "[3/4] Applying schema.sql to $DB_NAME..."
gcloud compute ssh "$INSTANCE_NAME" --zone="$ZONE" --project="$PROJECT_ID" \
  --command="cat /tmp/schema.sql | sudo docker exec -i $CONTAINER psql -U postgres -d $DB_NAME"
...
echo "======================================================================"
echo "SUCCESS: Remote Accounting Database $DB_NAME successfully initialized!"
echo "======================================================================"
