...
WITH RECURSIVE bom_tree AS (
    ...
    FROM mrp_bom_item bi JOIN mrp_bom parent_bom ON parent_bom.bom_no = bi.bom_no
        ...
        AND (parent_bom.effective_from IS NULL OR parent_bom.effective_from <= CURRENT_DATE)
        AND (parent_bom.effective_to IS NULL OR parent_bom.effective_to >= CURRENT_DATE)
    UNION ALL
    ...
    JOIN mrp_bom child_bom ON child_bom.bom_no = child_bi.bom_no
        ...
        AND (child_bom.effective_from IS NULL OR child_bom.effective_from <= CURRENT_DATE)
        AND (child_bom.effective_to IS NULL OR child_bom.effective_to >= CURRENT_DATE)
)
SELECT root_bom_no, item_code, item_name, SUM(required_qty)::NUMERIC(15,4) AS total_exploded_qty,
    uom, standard_rate, SUM(amount)::NUMERIC(15,4) AS total_exploded_amount, level_depth, path
