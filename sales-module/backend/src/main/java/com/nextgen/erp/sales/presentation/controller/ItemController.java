...
public class ItemController {
    ...
    public ResponseEntity<Item> updateItem(@PathVariable UUID id, @RequestBody Item updated) {
        return itemRepository.findById(id).map(existing -> {
            ...
            existing.setIsFixedAsset(updated.getIsFixedAsset());
            existing.setAllowAlternativeItem(updated.getAllowAlternativeItem());
            ...
            existing.setValuationMethod(updated.getValuationMethod());
            ...
            existing.setHasExpiryDate(updated.getHasExpiryDate());
            existing.setShelfLifeInDays(updated.getShelfLifeInDays());
            existing.setWarrantyPeriod(updated.getWarrantyPeriod());
            existing.setWeightPerUnit(updated.getWeightPerUnit());
            existing.setWeightUom(updated.getWeightUom());
            ...
            existing.setDefaultSupplier(updated.getDefaultSupplier());
            existing.setDeliveredBySupplier(updated.getDeliveredBySupplier());
            existing.setGrantCommission(updated.getGrantCommission());
            existing.setEnableDeferredRevenue(updated.getEnableDeferredRevenue());
            existing.setEnableDeferredExpense(updated.getEnableDeferredExpense());
            existing.setIncludeItemInManufacturing(updated.getIncludeItemInManufacturing());
            existing.setIsSubContractedItem(updated.getIsSubContractedItem());
            existing.setDefaultBom(updated.getDefaultBom());
            existing.setProductionCapacity(updated.getProductionCapacity());
            existing.setInspectionRequiredBeforePurchase(updated.getInspectionRequiredBeforePurchase());
            existing.setInspectionRequiredBeforeDelivery(updated.getInspectionRequiredBeforeDelivery());
            existing.setQualityInspectionTemplate(updated.getQualityInspectionTemplate());
            existing.setVariantBasedOn(updated.getVariantBasedOn());
